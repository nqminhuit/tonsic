...
const SHARP_PREFERRED_PCS = new Set([1, 6]);
const FLAT_PREFERRED_PCS = new Set([3, 8, 10]);
...
// Human-readable degree labels for common chord types (used in learning mode)
const CHORD_DEGREES = {
  maj: ['1','3','5'],
  min: ['1','b3','5'],
  maj7: ['1','3','5','7'],
...
function getChordDegrees(type) {
  return CHORD_DEGREES[type] ? CHORD_DEGREES[type].slice() : (CHORD_DEGREES.maj.slice());
}
...
function chooseAccidentalStyle(pcs = [], accidentalStyle = 'auto') {
  if (accidentalStyle === 'sharps' || accidentalStyle === 'flats') return accidentalStyle;

  const normalized = [...new Set((pcs || []).map(normalizePitchClass))];
  const sharpWeight = normalized.reduce((count, pc) => count + (SHARP_PREFERRED_PCS.has(pc) ? 1 : 0), 0);
  const flatWeight = normalized.reduce((count, pc) => count + (FLAT_PREFERRED_PCS.has(pc) ? 1 : 0), 0);

  if (flatWeight === 0 && sharpWeight === 0) return 'sharps';
  if (flatWeight > sharpWeight) return 'flats';
  return 'sharps';
}

function noteNameForPitchClass(pc, accidentalStyle = 'auto', pcs = []) {
  const style = chooseAccidentalStyle(pcs.length ? pcs : [pc], accidentalStyle);
  const names = style === 'flats' ? NOTE_NAMES_FLATS : NOTE_NAMES;
  return names[normalizePitchClass(pc)];
}

function noteNameForMidi(midi, accidentalStyle = 'auto', midis = []) {
  const pitchClass = normalizePitchClass(midi);
  const name = noteNameForPitchClass(pitchClass, accidentalStyle, midis);
  const octave = Math.floor(Math.floor(midi) / 12) - 1;
  return `${name}${octave}`;
}
...
function buildChord(root, type) {
  ...
  return formula.map((off) => normalizePitchClass(root + off));
}

function chordName(root, type) {
  const n = normalizePitchClass(root);
  return type === 'maj' ? NOTE_NAMES[n] : NOTE_NAMES[n] + type;
}
...
function getChordInfo(root, type, rootOctave = 4) {
  ...
  const names = pcs.map((p) => noteNameForPitchClass(p, 'sharps', pcs));
...
function matchExactVoicing(expectedMidis, playedMidis) {
  ...
  let matches = 0;
  const mismatches = [];
  const expectedStyle = chooseAccidentalStyle(expected, 'auto');
  const playedStyle = chooseAccidentalStyle(played, 'auto');
  ...
  for (let i = 0; i < len; i++) {
    if (played[i] === expected[i]) matches++;
      ...
      mismatches.push({
        index: i,
        expected: expected[i],
        expectedName: noteNameForMidi(expected[i], expectedStyle, expected),
        played: played[i],
        playedName: Number.isFinite(played[i]) ? noteNameForMidi(played[i], playedStyle, played) : null,
      });
    }
  }
...
if (typeof window !== 'undefined') {
  window.Tonsic = { randomChord, matchChord, buildChord, CHORD_FORMULAS, NOTE_NAMES, NOTE_NAMES_FLATS, getChordDegrees, chooseAccidentalStyle, noteNameForPitchClass, noteNameForMidi, buildVoicing, matchExactVoicing, setEnabledChordTypes, getEnabledChordTypes, getChordInfo };
}
